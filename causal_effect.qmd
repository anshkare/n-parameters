...
scale_y_continuous(breaks = c(0,1), labels = c("no", "yes"))+ labs(title = "Who Voted")
```
...
```
```{r}
fit_postcard_vote|> tbl_regression(intercept = TRUE, estimate_fun = function(x) style_sigfig(x, digits =3))
...
plot_data<-fit_postcard_vote|> add_epred_draws(newdata=ndata)|> ungroup()|>
select(names, .epred)|> pivot_wider(names_from = names, values_from = .epred, values_fn = list)|>
...
labs(title = "Treatment effect on Probability of Voting", subtitle = "Postcards work less well on those who rarely vote", x="Average Treatment effect", y = "Postcard Type")


```


We created a preceptor table with our quantity of interest defined, with the shaming data set. We identified validity of the dataset. We determined the creation of the population table, and assumed stability, representativeness, and unconfoundedness. Data from michingan. would be different from texas, and 2006 data cannot predict 2026 data because of changing political views.We then created a DGM with posterior results showing us that receiving a postcard with civic duty had a 1.6% more likely chance to vote. People who voted before will more likely vote again. We used our DGM to answer our question and present a easy way to show our data with a graph. It is an informed estimate based on the most relevant data possible. From that data, we have created a 95% confidence interval for the treatment effect of various postcards: between 8% to 10%.
